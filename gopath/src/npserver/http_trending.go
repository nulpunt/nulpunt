...
func updateTrendingHandlerFunc(rw http.ResponseWriter, req *http.Request) {
	...
	for iterator.Next(&document) {
		...
		annotations, err := getAnnotations(bson.M{"documentId": document.ID})
		log.Printf("error getting annotations for Doc: %#v\n", document.ID)
		trend := Trending{
			Document:    document,
			Annotations: annotations,
		}

		err = updateTrend(&trend) // update or insert
		if err != nil {
			// it's probably a new one
			// TODO: check for the correct message, or error code.
			log.Printf("error updating trend: %#v\n", err)
		}
